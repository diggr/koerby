...
def get_platform_groups():
    """ reads in platform groups file and returns it as a dict """
    groups = {}
    file_name = os.path.join(os.path.dirname(__file__), "resources/platform_groups.csv")
    df = pd.read_csv(file_name, sep=",")
    for col in df.columns:
        groups[col] = list(df[col].dropna())
    return groups
